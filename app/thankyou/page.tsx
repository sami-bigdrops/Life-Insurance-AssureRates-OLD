...
export default function ThankYouPage() {
  ...
  const [mortgageZipCode, setMortgageZipCode] = useState('')
  const [mortgageZipCodeError, setMortgageZipCodeError] = useState<string>('')
  ...
  const fetchUserLocation = useCallback(async () => {
    try {
      ...
      if (data.zipCode) {
        // Set ZIP code for all inputs
        setAutoZipCode(data.zipCode)
        setHomeZipCode(data.zipCode)
        setMortgageZipCode(data.zipCode)
      } else {
  ...
  const handleMortgageContinue = () => {
    const validation = validateZipCode(mortgageZipCode)
    if (!validation.valid) {
      setMortgageZipCodeError(validation.error || 'Please enter a valid 5-digit ZIP code')
      return
    }
    ...
    // Build the redirect URL
    const baseUrl = 'https://mortgage.assurerates.com'
    const params = new URLSearchParams({
      zip_code: mortgageZipCode,
    })
    ...
    const redirectUrl = `${baseUrl}/form/refinance?${params.toString()}`
  ...
  const handleMortgageKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      handleMortgageContinue()
    }
  }

  return (
    <div className="min-h-screen bg-linear-to-br from-blue-50 via-white to-blue-50 py-4 md:py-8 px-4">
      <div className="w-full max-w-6xl mx-auto">
        {/* Main Thank You Section */}
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden mb-6 md:mb-8">
          <div className="bg-linear-to-r from-[#246a99] to-[#3498DB] px-6 md:px-10 py-8 md:py-10 text-center">
            {/* Success Icon */}
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="bg-white rounded-xl md:rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-100 group flex flex-col">
              {/* Logo Header */}
              <div className="bg-linear-to-br from-blue-500 via-blue-400 to-blue-500 px-5 md:px-6 py-5 md:py-6">
              ...
              <div className="p-5 md:p-6 flex flex-col h-full">
                ...
                <div className="block sm:hidden space-y-4">
                  <input
                    ...
                    className={`w-full px-4 py-4 text-gray-900 text-[18px] font-semibold rounded-lg border transition-all duration-200 h-14 bg-white focus:outline-none focus:ring-2 ${
                      autoZipCodeError
                  ...
                  >
                    {isLoadingLocation ? (
                      ...
                      <>
                        Continue <ArrowRight className='w-4 h-4 text-white font-semibold' />
                      </>
                    )}
                  </button>
                </div>
                ...
                <div className="hidden sm:block">
                  <div className="relative">
                    <input
                      ...
                      onKeyPress={handleAutoKeyPress}
                      disabled={isLoadingLocation}
                      className={`w-full px-4 py-4 pr-24 text-gray-900 text-[18px] font-semibold rounded-lg border transition-all duration-200 h-14 bg-white focus:outline-none focus:ring-2 ${
                        autoZipCodeError
                          ? 'border-red-500 focus:ring-red-500 focus:border-red-500'
                          : 'border-gray-300 focus:ring-orange-400 focus:border-orange-400'
                      } ${
                    ...
                    <button 
                      onClick={handleAutoContinue}
                      disabled={isLoadingLocation || !validateZipCode(autoZipCode).valid}
                      className={`absolute right-0 top-0 px-6 py-2 rounded-r-lg font-semibold transition-all duration-200 flex items-center gap-2 text-[18px] h-14 text-white ${
                        isLoadingLocation || !validateZipCode(autoZipCode).valid
                          ? 'bg-gray-400 cursor-not-allowed' 
                          : 'bg-[#F7782B] hover:bg-[#e06c27]'
                      }`}
            ...
            <div className="bg-white rounded-xl md:rounded-2xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300 border border-gray-100 group flex flex-col">
              ...
              <div className="p-5 md:p-6 flex flex-col h-full">
                <h3 className="text-xl md:text-2xl font-bold text-[#246a99] mb-3 font-sans text-center">
                  AssureRates Mortgage
                </h3>
                
                <p className="text-gray-600 mb-4 text-sm md:text-base leading-relaxed text-center">
                  Compare rates from top insurers and protect your home in seconds. Peace of mind made easy.
                </p>
                ...
                <div className="block sm:hidden space-y-4">
                  <input
                    type="text"
                    placeholder={isLoadingLocation ? 'Detecting your location...' : 'Zip Code e.g. 11102'}
                    value={mortgageZipCode}
                    onChange={(e) => {
                      const value = e.target.value
                      if (/^\d{0,5}$/.test(value)) {
                        setMortgageZipCode(value)
                        if (mortgageZipCodeError) {
                          setMortgageZipCodeError('')
                        }
                      }
                    }}
                    onBlur={() => {
                      const validation = validateZipCode(mortgageZipCode)
                      if (!validation.valid) {
                        setMortgageZipCodeError(validation.error || '')
                      } else {
                        setMortgageZipCodeError('')
                      }
                    }}
                    onKeyPress={handleMortgageKeyPress}
                    disabled={isLoadingLocation}
                    className={`w-full px-4 py-4 text-gray-900 text-[18px] font-semibold rounded-lg border transition-all duration-200 h-14 bg-white focus:outline-none focus:ring-2 ${
                      mortgageZipCodeError
                        ? 'border-red-500 focus:ring-red-500 focus:border-red-500'
                        : 'border-gray-300 focus:ring-orange-400 focus:border-orange-400'
                    } ${
                      isLoadingLocation ? 'opacity-50 cursor-not-allowed' : ''
                  ...
                  />
                  {mortgageZipCodeError && (
                    <p className="mt-2 text-sm text-red-600 font-medium">{mortgageZipCodeError}</p>
                  )}
                  <button 
                    onClick={handleMortgageContinue}
                    disabled={isLoadingLocation || !validateZipCode(mortgageZipCode).valid}
                    className={`w-full px-4 py-4 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center gap-2 text-[18px] h-14 text-white ${
                      isLoadingLocation || !validateZipCode(mortgageZipCode).valid
                        ? 'bg-gray-400 cursor-not-allowed' 
                        : 'bg-[#F7782B] hover:bg-[#e06c27]'
                    }`}
                  >
                ...
                <div className="bg-blue-50 rounded-lg p-4 mt-4 border-2 border-blue-200">
                  ...
                  <p className="text-sm md:text-base text-blue-800 text-center font-semibold">
                    Over 1 million families get mortgage protection yearly.
                  </p>
                </div>
              </div>
            </div>
